...
// TIME_RANGES is initialized in the component to access t() for labels
...
export function Logs(): JSX.Element {
  ...
  // Define TIME_RANGES with translated labels
  const TIME_RANGES: readonly TimeRange[] = [
    { label: t('last_hour'), since: Date.now() - 60 * 60 * 1000, until: undefined },
    { label: t('last_24h'), since: Date.now() - 24 * 60 * 60 * 1000, until: undefined },
    { label: t('last_7d'), since: Date.now() - 7 * 24 * 60 * 60 * 1000, until: undefined },
    { label: t('last_30d'), since: Date.now() - 30 * 24 * 60 * 60 * 1000, until: undefined },
    { label: t('any'), since: undefined, until: undefined },
  ];
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <Button variant={logs.live ? 'primary' : 'secondary'} size="sm" onClick={logs.toggleLive}>
            {logs.live ? t('live_button_on') : t('live_button_off')}
          </Button>
          {logs.live && (
      ...
      <Card>
        ...
        <CardBody className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {t('export_csv', { count: selectedIds.size > 0 ? ` (${selectedIds.size})` : '' })}
            </Button>
            <Button
              variant="secondary"
              size="sm"
              ...
              disabled={filteredLogs.length === 0}
            >
              {t('export_json', { count: selectedIds.size > 0 ? ` (${selectedIds.size})` : '' })}
            </Button>
            {selectedIds.size > 0 && (
        ...
        <Card className="border-status-error/40">
          <CardBody>
            <p className="text-sm text-status-error">
              {logs.live ? t('live_stream_error') : t('load_error')}
              {logs.error.message}
            </p>
          </CardBody>
        </Card>
