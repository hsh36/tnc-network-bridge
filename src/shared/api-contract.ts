...
export const apiContract = {
  ...
  'osUpdate.status': {
    method: 'GET',
    path: '/os-update/status',
    summary: 'Progress and outcome of the last Raspberry Pi OS package update.',
    auth: 'session-or-token',
    response: osUpdateStatusSchema,
  },
  'osUpdate.run': {
    method: 'POST',
    path: '/os-update/run',
    summary: 'Install Raspberry Pi OS package updates now.',
    auth: 'session',
    body: runOsUpdateRequestSchema,
    response: acceptedSchema,
    mutates: true,
  },
  'update.history': {
  ...
  'certificates.download': {
    ...
    summary: 'Download the public certificate, with its chain. Never the private key.',
    auth: 'session',
    response: z.unknown(),
    produces: 'binary',
    unenveloped: true,
